...
class BusinessLogic:
    def __init__(self, storage: JsonStorage | TxtStorage) -> None:
        self.storage = storage

    @staticmethod
    def validate_time(raw_time: str) -> None:
        fmt = "%H:%M"
    ...
    def chose_or_create_day(self, day_date: str) -> list[Note]:
    ...
    @staticmethod
    def return_today_date() -> str:
        today = str(datetime.date(datetime.now()))
        return today

    def find_insert_index(self, day_notes: list[Note], time: str) -> int:
        fmt = "%H:%M"
    ...
    def write_notes(self, insert_pos: int, new_note: str, time: str, day_date: str, day_notes: list[Note]) -> None:
        time = time.split("-")
    ...
    def edit_notes_text(self, note_index: int, new_note: str, day_date: str) -> None:
        notes = self.storage.read_day(day_date)
    ...
    def edit_notes_time(self, note_index: int, new_time: str, day_date: str, day_notes: list[Note]) -> None:
        notes = self.storage.read_day(day_date)
    ...
    def delete_note(self, note_index: int, day_date: str) -> None:
        notes = self.storage.read_day(day_date)
    ...
    def migrate_notes(self, old_storage: JsonStorage | TxtStorage, new_storage: JsonStorage | TxtStorage) -> dict[str, list[Note]]:
        old_storage.check_changes()
        log_for_migrated_notes: dict[str, list[Note]] = {}
